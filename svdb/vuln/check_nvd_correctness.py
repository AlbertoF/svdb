...
def test_cvss(root):
    ...
    for entry in root.iter(dict_nsmap['entry']):
        ...
        if entry.find(dict_nsmap['vuln:cvss']) is None:
...
def test_has_vulnerable_configuration_and_vulnerable_software_list(root):
    ...
    for entry in root.iter(dict_nsmap['entry']):
        ...
        if entry.find(dict_nsmap['vuln:vulnerable-configuration']) is None:
            summary = entry.find(dict_nsmap['vuln:summary'])
            if not summary is None:
                if "** REJECT **" in summary.text:
                    continue
            bad_cve_id_list.add(cve_id)
            
            if entry.find(dict_nsmap['vuln:vulnerable-software-list']) is not None:
                pass
        ...
        elif entry.find(dict_nsmap['vuln:vulnerable-software-list']) is None:
...
def test_multiple_vulnerable_configuration(root):
    ...
    for entry in root.iter(dict_nsmap['entry']):
        cve_id = entry.get('id')
        if len(entry.findall(dict_nsmap['vuln:vulnerable-configuration'])) > 1 :
...
def test_has_logical_elem(root):
    ...
    for vuln_cfg in root.iter(dict_nsmap['vuln:vulnerable-configuration']):
        if vuln_cfg.find(dict_nsmap['cpe-lang:logical-test']) is None:
...
def test_and_inside_or(root):
    ...
    for vuln_cfg in root.iter(dict_nsmap['vuln:vulnerable-configuration']):
        cve_id = vuln_cfg.getparent().get('id')
        for lang in vuln_cfg.iter(dict_nsmap['cpe-lang:logical-test']):
            if is_or_elem(lang):
                found = False
                for elem in lang:
                    if is_and_elem(elem):
